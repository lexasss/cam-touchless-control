...
namespace LeapInternal
{
    ...
    public class Connection
    {
        ...
        {
            if (!connectionDictionary.TryGetValue(connectionKey, out Connection? conn))
            {
        ...
        //Connection events
        public SynchronizationContext EventContext { get; set; } = new();
        ...
        {
            _configRequests.TryGetValue(configEvent.requestId, out string? config_key);
            if (config_key != null)
        ...
        private void handleConfigResponse(ref LEAP_CONNECTION_MESSAGE configMsg)
        {
            ...
            _configRequests.TryGetValue(config_response_evt.requestId, out string? config_key);
            if (config_key != null)
        ...
        private void reportLogMessage(ref LEAP_LOG_EVENT logMsg)
        {
            ...
            {
                LeapLogEvent.DispatchOnContext(this, EventContext, new LogEventArgs(publicSeverity(logMsg.severity),
                    logMsg.timestamp, Marshal.PtrToStringAnsi(logMsg.message) ?? ""));
            }
        }
